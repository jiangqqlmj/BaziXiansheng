 ...
 */
public class MyUserAdapter extends RecyclerView.Adapter<MyUserAdapter.RecyclerHolder> {
    /**
     * 1 inflater
     * 2context
     * 3datas   get set方法
    ...
    public MyUserAdapter(Context mContext, IClickListener mCallBack) {
...
//        }
        if (!TextUtils.isEmpty(bean.getUsername())) {
            holder.augur_name.setText(bean.getUsername());
        } else {
            holder.augur_name.setText("无名大师");
        }
        if (bean != null) {
            if (TextUtils.isEmpty(bean.getGoldNum() + "")) {
                holder.gold_num.setText("0");
            } else {
                holder.gold_num.setText(bean.getGoldNum() + "");
            }
        } else {
            holder.gold_num.setText("0");
        }
        if (bean.getAvatar() == null) {
            ...
            //加载网络图片  没写
            bean.getAvatar().loadImageThumbnail(mContext, holder.iv_bg, 150, 150, 100);
        }
        //测算状态标记为判断  ing done
        if (bean.getOnlineStatus() != null) {
            if (bean.getOnlineStatus()) {
